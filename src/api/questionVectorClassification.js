// Phase 4: vector-based question → chapter classification using syllabus KB.

import { fetchEmbeddings } from "./syllabusKnowledgeExtraction.js";
import {
...
/** Sharpen top-3 softmax; raw cosine scores sit in ~0.25–0.65. */
...
function chapterRawScore(questionEmbedding, profile) {
  ...
  // Best concept match often dominates for exam questions; blend with summary.
  const blended =
...
function rankChaptersForQuestion(questionEmbedding, kbProfiles, chapterIndex) {
  ...
  // Confidence is relative among the top candidates only — not divided across
  // every chapter in the class (which flattened scores to ~5–15% each).
...
/**
 * Classify questions using syllabus KB embeddings.
 * @returns {Promise<{ questions: Array, classifiedBy: string }>}
 */
export async function classifyQuestionsWithVectorKb(
  ...
  const classified = questions.map((q, index) => {
    ...
    if (!qEmb) {
      return {
        ...q,
        chapterId: chapterIndex[0]?.id ?? null,
        chapterName: chapterIndex[0]?.name ?? null,
        unitId: chapterIndex[0]?.unitId ?? null,
...
export { cosineSimilarity, rankChaptersForQuestion };
