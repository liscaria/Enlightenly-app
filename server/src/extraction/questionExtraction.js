...
// Uses OpenAI when OPENAI_API_KEY is set; otherwise PDF/text heuristics.

import * as pdfjs from "pdfjs-dist/legacy/build/pdf.mjs";
import path from "path";
import { fileURLToPath } from "url";
import { buildExtractionMessages, EXTRACTION_SYSTEM_PROMPT, buildVisionExtractionUserText, buildMissingQuestionsRetryPrompt } from "./extractionPrompt.js";
...
async function pdfBlobToPageImages(blob) {
  ...
  for (let pageNum = 1; pageNum <= limit; pageNum += 1) {
    ...
    const jpegBuf = canvas.toBuffer("image/jpeg", 82);
    dataUrls.push(`data:image/jpeg;base64,${jpegBuf.toString("base64")}`);
...
function formatExtractionFailureReason({
  ...
  if (!aiConfigured) {
    return "OPENAI_API_KEY is not configured on the extraction server.";
  }
  if (aiError) {
    return aiError;
  }
